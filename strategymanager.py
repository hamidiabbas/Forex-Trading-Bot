...
class StrategyManager:
    ...
    def evaluate_trend_following_strategy(self, symbol, df, i):
        ...
        if i < getattr(self.config, 'TREND_EMA_SLOW_PERIOD', 50):
            return None

        try:
            fast_ema_col = f"EMA_{getattr(self.config, 'TREND_EMA_FAST_PERIOD', 20)}"
            slow_ema_col = f"EMA_{getattr(self.config, 'TREND_EMA_SLOW_PERIOD', 50)}"
            rsi_col = f"RSI_{getattr(self.config, 'RSI_PERIOD', 14)}"
            macd_col = "MACD_12_26_9"
            macd_signal_col = "MACDs_12_26_9"

            if not all(col in df.columns for col in [fast_ema_col, slow_ema_col, rsi_col]):
                return None
    ...
    def evaluate_mean_reversion_strategy(self, symbol, df, i):
        ...
        try:
            bb_upper_col = f"BBU_{getattr(self.config, 'BBANDS_PERIOD', 20)}_{getattr(self.config, 'BBANDS_STD', 2.0)}"
            bb_lower_col = f"BBL_{getattr(self.config, 'BBANDS_PERIOD', 20)}_{getattr(self.config, 'BBANDS_STD', 2.0)}"
            rsi_col = f"RSI_{getattr(self.config, 'RSI_PERIOD', 14)}"

            if not all(col in df.columns for col in [bb_upper_col, bb_lower_col, rsi_col]):
                return None
            ...
            if (prev_high < df[bb_upper_col].iloc[i-1] and last_high > upper_band and 
                last_rsi > getattr(self.config, 'RANGE_RSI_OVERBOUGHT', 70)):
            ...
            if (prev_low > df[bb_lower_col].iloc[i-1] and last_low < lower_band and 
                last_rsi < getattr(self.config, 'RANGE_RSI_OVERSOLD', 30)):
    ...
    def generate_signal(self, symbol, direction, strategy, df, i):
        ...
        try:
            return {
                ...
                "atr_at_signal": df.get("ATRr_14", pd.Series([0.001])).iloc[i]
            }
        except Exception as e:
