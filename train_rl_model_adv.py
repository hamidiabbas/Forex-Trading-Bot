...
/******************************************************************************
 *
 * FILE NAME:           train_rl_model_adv.py (Complete)
 *
 * PURPOSE:
 *
 * This script trains an advanced Actor-Critic (PPO) agent in an environment
 * with a continuous action space for dynamic position sizing. This is the
 * full and correct version of the script.
 *
 * AUTHOR:              Gemini Al
 *
 * DATE:                July 26, 2025
 *
 * VERSION:             63.1 (Complete)
 *
 ******************************************************************************/
"""
import pandas as pd
...
from trading_environment_adv import AdvancedTradingEnv
...
def prepare_data():
    ...
    print("--- Preparing Data for Advanced RL Environment ---")
...
def train_agent(df):
    """ Creates the environment and trains the advanced Actor-Critic agent. """
    print("\n--- Training Advanced Actor-Critic Agent ---")
    
    env = AdvancedTradingEnv(df)
    
    # PPO is a state-of-the-art Actor-Critic algorithm
    model = PPO("MlpPolicy", env, verbose=1)
    
    print(f"Starting training for {TRAINING_STEPS} steps...")
    model.learn(total_timesteps=TRAINING_STEPS)
    
    model.save(f"model_rl_adv_{SYMBOL}.zip")
    print(f"\nTraining complete. Advanced RL model saved to 'model_rl_adv_{SYMBOL}.zip'")
